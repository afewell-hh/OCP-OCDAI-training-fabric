# OPG-64 — Mesh-Converged Rail-Optimized (CX7 1x400G, BF3 2x200G, Converged Storage, In-Band 2x25G)

This building block provides a 64-xPU training cluster using a shared DS5000 mesh pair that carries scale-out, SoC/storage, and in-band management on a single converged fabric. Scale-out traffic uses CX7 1x400G per GPU with rail-optimized distribution; storage and SoC traffic use BF3 2x200G per server; in-band management uses a dedicated DS2000 pair with a generic 2x25G NIC on every server class. Out-of-band management uses a DS1000 pair. Explicit named fabrics (`soc-storage-scale-out`, `inb-mgmt`, `oob-mgmt`) provide clean separation without increasing the managed switch count above what is needed.

## Intended use

- Choose this OPG when rail-aware scale-out placement is required at the 64-xPU tier with a minimal switch count.
- Use when a separate in-band management fabric (DS2000) is needed alongside the converged scale-out/SoC leaf pair.
- Use when SoC/storage traffic must share the same DS5000 mesh as scale-out, without a separate frontend fabric.
- The rail-optimized sibling of the mesh-conv-sh variant; choose rail-optimized when job-scheduler alignment with first-hop leaf domains is expected to reduce spine traffic.
...
- Topology type: mesh-converged, rail-optimized scale-out
- Scale-out NICs: CX7 1x400G per GPU (8 ports per compute server, one per rail)
...
- Multihoming: rail-optimized distribution on scale-out; same-switch on SoC/storage
...
- [`xoc-64/1x-OPG-64/mesh-conv-ro--cx7-1x400g--bf3-2x200g--storage-conv-2x200g--inb-2x25g`](../../../xoc/xoc-64/1x-OPG-64/mesh-conv-ro--cx7-1x400g--bf3-2x200g--storage-conv-2x200g--inb-2x25g/)
- [`xoc-128/2x-opg-64/mesh-conv-ro--cx7-1x400g--bf3-2x200g--storage-conv-2x200g--inb-2x25g`](../../../xoc/xoc-128/2x-opg-64/mesh-conv-ro--cx7-1x400g--bf3-2x200g--storage-conv-2x200g--inb-2x25g/)
